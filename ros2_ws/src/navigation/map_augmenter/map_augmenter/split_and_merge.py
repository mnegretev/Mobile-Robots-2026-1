...
FULL_NAME = "Angel Anduaga Balestrini"
...
class SplitAndMergeNode(Node):
    ...
    def split(self, points, threshold, min_points):
        ...
        #
        if len(points) < min points:
            return lines
        rho , theta, xm, ym, length = self.adjust_line_by_LSE(points)
        idx , dist = self.find_farthest_point(points, rho, theta)
        if dist < threshold:
            return [[rho, theta, xm, ym, length]]
        lines1 = self.split(points[ 0: idx], threshold, min points)
        lines2 = self.split(points[ idx+1: len (points)], threshold, min points)
        lines = lines1 + lines2        
        return lines
