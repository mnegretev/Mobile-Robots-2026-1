...
#define FULL_NAME "Popoca Zuñiga Daniel Ixbalanque"
...
public:
	...
	min_y = this->declare_parameter<double>("min_y", -12.0);
	...
	sub_scan = this->create_subscription<sensor_msgs::msg::LaserScan>(
	    "/scan", 1,std::bind(&ParticleFilterNode::scan_callback, this, std::placeholders::_1));
	...
	while(rclcpp::ok()){
		...
		RCLCPP_INFO(this->get_logger(), "Robot pose: x=%lf\ty=%lf\tw=%lf\tz=%lf",
			    tf_odom_to_map.transform.translation.x, tf_odom_to_map.transform.translation.y,
			    tf_odom_to_map.transform.rotation.w, tf_odom_to_map.transform.rotation.z);
	    }
